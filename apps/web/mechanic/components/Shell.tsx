...
const ROLE = { label: 'Mechanic', tint: 'bg-emerald-500/15 text-emerald-200 ring-emerald-400/30' };

const NAV = [
  { href: '/', label: 'Dashboard', icon: '◎' },
  { href: '/jobs', label: 'Bookings & Jobs', icon: '🔧' },
  { href: '/nearby', label: 'Roadside Jobs', icon: '🚨' },
...
function Wordmark({ light, compact }: { light?: boolean; compact?: boolean }) {
  return (
    <Link href="/" className="flex items-center gap-2.5">
      ...
      {!compact && (
        <span className={classNames('font-display text-lg font-extrabold tracking-tight', light ? 'text-white' : 'text-slate-900')}>AutoMate</span>
      )}
    </Link>
  );
}
...
function NotifBell() {
  ...
  const load = useCallback(async () => {
    try { setItems(await api.listNotifications()); } catch { /* */ }
  }, []);
  useEffect(() => { load(); }, [load]);
  useEffect(() => {
  ...
  return (
    <div ref={ref} className="relative">
      <button onClick={() => setOpen((o) => !o)} aria-label="Notifications" className="relative grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white/70 text-lg transition hover:bg-white">
        🔔
        {unread > 0 && <span className="absolute -right-1 -top-1 grid h-5 min-w-[20px] place-items-center rounded-full bg-red-500 px-1 text-[10px] font-bold text-white ring-2 ring-white">{unread > 9 ? '9+' : unread}</span>}
      </button>
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: -8, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -6, scale: 0.98 }} transition={{ duration: 0.16 }} className="glass absolute right-0 z-50 mt-2 w-80 rounded-2xl p-2 shadow-xl">
    ...
    <div ref={ref} className="relative">
      <button onClick={() => setOpen((o) => !o)} className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white/70 py-1.5 pl-1.5 pr-3 transition hover:bg-white">
        ...
        <span className="hidden text-left sm:block">
          ...
          <span className="block text-[11px] text-slate-400">Mechanic</span>
        </span>
      ...
      <AnimatePresence>
        {open && (
          <motion.div initial={{ opacity: 0, y: -8, scale: 0.97 }} animate={{ opacity: 1, y: 0, scale: 1 }} exit={{ opacity: 0, y: -6, scale: 0.98 }} transition={{ duration: 0.16 }} className="glass absolute right-0 z-50 mt-2 w-56 rounded-2xl p-2 shadow-xl">
            ...
            <Link href="/profile" onClick={() => setOpen(false)} className="mt-1 flex items-center gap-2.5 rounded-xl px-3 py-2 text-sm text-slate-700 transition hover:bg-slate-100">👤 Profile & availability</Link>
            <Link href="/earnings" onClick={() => setOpen(false)} className="flex items-center gap-2.5 rounded-xl px-3 py-2 text-sm text-slate-700 transition hover:bg-slate-100">💰 Earnings</Link>
  ...
  return (
    <div className="min-h-screen lg:flex">
      <aside className={classNames('sticky top-0 hidden h-screen shrink-0 flex-col overflow-hidden border-r border-white/5 bg-slate-900 p-4 transition-[width] duration-300 lg:flex', collapsed ? 'w-20' : 'w-72')}>
        <div className="pointer-events-none absolute inset-0 opacity-60" style={{ backgroundImage: 'radial-gradient(30rem 30rem at 120% -10%, rgba(20,184,166,0.25), transparent 60%), radial-gradient(24rem 24rem at -10% 110%, rgba(6,182,212,0.18), transparent 60%)' }} />
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        <header className="sticky top-0 z-30 hidden items-center justify-between border-b border-slate-200/60 bg-white/70 px-6 py-3 backdrop-blur-xl lg:flex">
          <div className="flex items-center gap-3">
            <span className="rounded-full bg-emerald-50 px-3 py-1 text-xs font-semibold text-emerald-700 ring-1 ring-inset ring-emerald-600/20">Mechanic Workspace</span>
            <p className="text-sm text-slate-500">Welcome back{user?.name ? `, ${user.name.split(' ')[0]}` : ''} 🔧</p>
          </div>
          <div className="flex items-center gap-3">
            <Link href="/jobs" className="rounded-xl bg-aurora px-4 py-2 text-sm font-semibold text-white shadow-glow transition hover:brightness-110">View jobs</Link>
            <NotifBell />
            <ProfileMenu />
          </div>
        </header>
