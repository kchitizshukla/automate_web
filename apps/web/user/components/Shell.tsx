...
const ROLE = { label: 'Customer', tint: 'bg-indigo-500/15 text-indigo-200 ring-indigo-400/30' };

const NAV = [
  { href: '/', label: 'Dashboard', icon: '◎' },
  { href: '/discover', label: 'Find Mechanics', icon: '🔍' },
  { href: '/book', label: 'Book Service', icon: '🛠' },
  { href: '/services', label: 'My Services', icon: '📋' },
  { href: '/vehicles', label: 'My Garage', icon: '🚗' },
  { href: '/payments', label: 'Payments', icon: '💳' },
  { href: '/issues', label: 'Support', icon: '🎧' },
  { href: '/notifications', label: 'Notifications', icon: '🔔' },
  { href: '/profile', label: 'Profile', icon: '👤' },
];

// Mobile bottom-tab key actions
const TABS = [
...
function Wordmark({ light, compact }: { light?: boolean; compact?: boolean }) {
  return (
    <Link href="/" className="flex items-center gap-2.5">
      ...
      {!compact && (
        <span className={classNames('font-display text-lg font-extrabold tracking-tight', light ? 'text-white' : 'text-slate-900')}>
          AutoMate
        </span>
      )}
    </Link>
        ...
        return (
          <Link
            ...
            className={classNames(
              ...
              active
                ? 'bg-white/10 text-white shadow-glow ring-1 ring-white/15'
                : light
  ...
  return (
    <div ref={ref} className="relative">
      <button
        ...
        className="relative grid h-10 w-10 place-items-center rounded-xl border border-slate-200 bg-white/70 text-lg transition hover:bg-white"
      >
        🔔
        {unread > 0 && (
          <span className="absolute -right-1 -top-1 grid h-5 min-w-[20px] place-items-center rounded-full bg-red-500 px-1 text-[10px] font-bold text-white ring-2 ring-white">
            {unread > 9 ? '9+' : unread}
          </span>
        )}
      </button>
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            className="glass absolute right-0 z-50 mt-2 w-80 rounded-2xl p-2 shadow-xl"
          >
            <div className="flex items-center justify-between px-2 py-1.5">
              <span className="text-sm font-bold text-slate-800">Notifications</span>
              {unread > 0 && (
                <button onClick={markAll} className="text-xs font-semibold text-brand hover:underline">Mark all read</button>
              )}
            </div>
            ...
            <Link href="/notifications" onClick={() => setOpen(false)} className="mt-1 block rounded-xl py-2 text-center text-sm font-semibold text-brand hover:bg-brand/5">
              View all →
            </Link>
          </motion.div>
        )}
    ...
    <div ref={ref} className="relative">
      <button onClick={() => setOpen((o) => !o)} className="flex items-center gap-2 rounded-xl border border-slate-200 bg-white/70 py-1.5 pl-1.5 pr-3 transition hover:bg-white">
        ...
        <span className="hidden text-left sm:block">
          ...
          <span className="block text-[11px] text-slate-400">Customer</span>
        </span>
      ...
      <AnimatePresence>
        {open && (
          <motion.div
            ...
            exit={{ opacity: 0, y: -6, scale: 0.98 }}
            transition={{ duration: 0.16 }}
            className="glass absolute right-0 z-50 mt-2 w-56 rounded-2xl p-2 shadow-xl"
          >
            <div className="border-b border-slate-100 px-3 py-2">
              <p className="truncate text-sm font-semibold text-slate-800">{user?.name}</p>
              <p className="truncate text-xs text-slate-400">{user?.email}</p>
            </div>
            <Link href="/profile" onClick={() => setOpen(false)} className="mt-1 flex items-center gap-2.5 rounded-xl px-3 py-2 text-sm text-slate-700 transition hover:bg-slate-100">👤 My profile</Link>
            <Link href="/vehicles" onClick={() => setOpen(false)} className="flex items-center gap-2.5 rounded-xl px-3 py-2 text-sm text-slate-700 transition hover:bg-slate-100">🚗 My garage</Link>
            <Link href="/payments" onClick={() => setOpen(false)} className="flex items-center gap-2.5 rounded-xl px-3 py-2 text-sm text-slate-700 transition hover:bg-slate-100">💳 Payments</Link>
...
export function Shell({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    try { setCollapsed(localStorage.getItem('am_sidebar') === '1'); } catch { /* */ }
  }, []);
  function toggleCollapse() {
    setCollapsed((c) => {
      const next = !c;
      try { localStorage.setItem('am_sidebar', next ? '1' : '0'); } catch { /* */ }
      return next;
    });
  }
  ...
  return (
    <div className="min-h-screen lg:flex">
      ...
      <aside
        className={classNames(
          'sticky top-0 hidden h-screen shrink-0 flex-col overflow-hidden border-r border-white/5 bg-slate-900 p-4 transition-[width] duration-300 lg:flex',
          collapsed ? 'w-20' : 'w-72',
        )}
      >
        <div
          className="pointer-events-none absolute inset-0 opacity-60"
          style={{ backgroundImage: 'radial-gradient(30rem 30rem at 120% -10%, rgba(139,92,246,0.25), transparent 60%), radial-gradient(24rem 24rem at -10% 110%, rgba(14,165,233,0.18), transparent 60%)' }}
        />
        <div className={classNames('relative mb-7 flex items-center', collapsed ? 'justify-center' : 'justify-between px-1')}>
          ...
          {!collapsed && (
            <button onClick={toggleCollapse} aria-label="Collapse sidebar" className="grid h-7 w-7 place-items-center rounded-lg text-slate-400 transition hover:bg-white/10 hover:text-white">⟨</button>
          )}
        </div>
        {collapsed && (
          <button onClick={toggleCollapse} aria-label="Expand sidebar" className="relative mb-3 grid h-7 w-full place-items-center rounded-lg text-slate-400 transition hover:bg-white/10 hover:text-white">⟩</button>
        )}
        <div className="relative flex-1 overflow-y-auto">
          <NavLinks light collapsed={collapsed} />
        </div>
        {!collapsed && (
      ...
      <div className="flex min-w-0 flex-1 flex-col">
        ...
        <header className="sticky top-0 z-30 hidden items-center justify-between border-b border-slate-200/60 bg-white/70 px-6 py-3 backdrop-blur-xl lg:flex">
          <div className="flex items-center gap-3">
            <span className={classNames('rounded-full px-3 py-1 text-xs font-semibold ring-1 ring-inset', 'bg-indigo-50 text-indigo-700 ring-indigo-600/20')}>
              Customer Portal
            </span>
            <p className="text-sm text-slate-500">Welcome back{user?.name ? `, ${user.name.split(' ')[0]}` : ''} 👋</p>
          </div>
          <div className="flex items-center gap-3">
            <Link href="/book" className="rounded-xl bg-aurora px-4 py-2 text-sm font-semibold text-white shadow-glow transition hover:brightness-110">+ Book service</Link>
            <NotifBell />
            <ProfileMenu />
          </div>
        </header>
        ...
        <AnimatePresence>
          {open && (
            <motion.div
              ...
              className="overflow-hidden border-b border-white/10 bg-slate-900 lg:hidden"
            >
        ...
        <main className="flex-1 pb-20 lg:pb-0">
          <div className="w-full max-w-none px-4 py-7 sm:px-6 lg:px-8 lg:py-8">
