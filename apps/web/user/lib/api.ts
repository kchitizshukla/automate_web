...
export const TOKEN_KEY = 'am_user_token';

const baseUrl =
  process.env.NEXT_PUBLIC_USER_API || 'http://localhost:4000/api/user';
...
export const api: ApiClient = createClient({
  ...
  onUnauthorized: () => {
    ...
    window.localStorage.removeItem('am_user');
...
// Absolute base for image / upload paths served by the backend. The backend
// serves each module's uploads under that module's own API prefix
// (/api/user/uploads/...), so stored paths resolve against the API base itself.
export const apiOrigin = baseUrl.replace(/\/$/, '');
