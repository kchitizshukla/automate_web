...
export function StatCard({ label, value, icon, trend, accent = 'amber' }: { label: string; value: React.ReactNode; icon?: string; trend?: string; accent?: 'indigo' | 'violet' | 'sky' | 'emerald' | 'amber' | 'orange' | 'rose' }) {
  const rings: Record<string, string> = {
    ...
    orange: 'from-orange-500/15 to-orange-500/0 text-orange-600',
...
export function Th({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <th className={classNames('sticky top-0 z-10 whitespace-nowrap border-b border-slate-100 bg-white/80 px-4 py-3 text-left text-xs font-semibold uppercase tracking-wide text-slate-400 backdrop-blur', className)}>
      {children}
    </th>
  );
}
...
export function Td({ children, className }: { children: React.ReactNode; className?: string }) {
  return (
    <td className={classNames('whitespace-nowrap px-4 py-3.5 text-slate-600', className)}>
      {children}
    </td>
  );
}
